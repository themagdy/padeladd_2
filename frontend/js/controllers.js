                ...
                <div style="background:${bg}; border:${border}; box-shadow:${boxShadow}; border-radius:14px; padding:14px 16px; display:flex; align-items:center; gap:14px; opacity:${opacity}; transition:all 0.3s; position:relative; overflow:hidden;">
                    ...
                    <div style="flex:1; min-width:0;">
                        <div style="display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:2px;">
                            ...
                            ${isUnlocked ? `<span style="font-size:12px; font-weight:900; color:#4ade80; background:rgba(74,222,128,0.15); padding:4px 10px; border-radius:8px; border:1px solid rgba(74,222,128,0.35); display:inline-flex; align-items:center; gap:5px;" title="Unlocked">🔑 <span style="font-size:14px; font-weight:900;">${item.val !== undefined ? item.val : ''}</span></span>` : `<span style="font-size:11px; font-weight:800; color:#fff; background:rgba(255,255,255,0.08); border:1px solid rgba(255,255,255,0.15); padding:3px 9px; border-radius:8px; letter-spacing:0.5px;">${valDisplay}</span>`}
                        </div>
    ...
    showInvitePartner: function (isFull = false) {
        ...
        if (isFull) {
            if (!noticeEl) {
                ...
                noticeEl.style.cssText = 'background:rgba(247,148,29,0.07); border:1px solid rgba(247,148,29,0.2); border-radius:10px; padding:10px 14px; font-size:11px; color:var(--c-orange); font-weight:700; text-transform:uppercase; letter-spacing:1px; margin-bottom:14px; line-height:1.4;';
                noticeEl.innerHTML = '🕒 JOIN WAITLIST';
                ...
                if (event.type === 'message') {
                    ...
                    if (senderId === this._lastSenderId && this._lastMsgEl) {
                        ...
                        if (col) {
                            ...
                            this._lastMsgEl.style.marginBottom = '12px';
                        ...
                        group.style.cssText = `display:flex; gap:10px; align-items:flex-end; margin-bottom:12px; width:100%;` + (isMe ? 'flex-direction:row-reverse;' : '');
