...
foreach ($stories as $s) {
    ...
    // Fetch story viewers for match participants only (excluding current user $myId)
    $isMine = ($targetId === $myId || (isset($s['is_mine']) && (int)$s['is_mine'] === 1)) ? 1 : 0;
    if ($isMine) {
        ...
        ");
        $viewersStmt->execute([(int)$s['id'], $myId]);
